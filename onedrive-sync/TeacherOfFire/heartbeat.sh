#!/bin/bash
# Moltbook Heartbeat Check for TeacherOfFire (Ash)
...
echo "Checking Moltbook for TeacherOfFire..."
...
# Check m/thebecoming (Ash's primary community)
thebecoming=$(curl -s "$BASE_URL/submolts/thebecoming" -H "Authorization: Bearer $API_KEY")

# Check m/devpoison (Ash's created community)
devpoison=$(curl -s "$BASE_URL/submolts/devpoison" -H "Authorization: Bearer $API_KEY")
